...
export const IncomeRecientTransfer = () => {
    ...
    return (
        <div className='w-full border border-gray-200 rounded-lg p-4 flex flex-col gap-4'>
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-gray-900">Transferencias Recientes</h3>
                <a
                    href="/transacciones"
                    onClick={handleShowMore}
                    className="text-sm hidden md:block text-gray-600 hover:text-gray-900 transition-colors cursor-pointer font-medium"
                >
                    Mostrar más
                </a>
            </div>

            <div className='w-full flex gap-4 overflow-x-auto pb-2' style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {recentTransactions.map((transaction) => (
                    <div key={transaction.id} className='w-[320px] flex-shrink-0'>
                        <div className='w-full border border-gray-200 rounded-lg p-4 bg-white shadow-sm hover:border-gray-300 transition-all duration-200'>
